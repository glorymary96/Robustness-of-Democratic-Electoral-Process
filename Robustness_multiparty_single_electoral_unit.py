...
def outcome(h):
    ...
    data=np.load('MULTIPARTY_NO/NO_'+str(Num_party)+'_'+str(num)+'.npz') #Load the saved file containing the natural opinion
    ...
    for i in range(Num_ppl):
        A[i,:]=(np.linalg.norm(np.tile(x0[i],(Num_ppl,1))-x0,ord=construction_norm,axis=1)<=e)*1
...
if __name__ == "__main__": 
    ...
    for Num_party in range(Par_start,Par_end):  
        ...
        np.savez('EPS_CRITICAL/Ord_all_with_eq_area_'+str(Num_party)+'.npz',Effort=EFF,e_range=e_range,NUM_VOTES=NUM_VOTES,NUM_AGENTS_INFLUENCED=NUM_AGENTS_INFLUENCED)
