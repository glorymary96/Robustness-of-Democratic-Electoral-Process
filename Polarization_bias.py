...
def outcome(h): 
 ...
 # Random Influence
   ...
   # Influence by minimum value 
    z = np.argsort(x0 + 1e2 * (x0 < 0)*(-x0)) # Indices of the agents near to the neutral opinion
    p=P_ini.copy()
    n=N_ini.copy() 
    y=Y.copy()
    iter=0
    ...
    np.savez('DATA/Res_'+str(h)+'.npz', pos=d,nos=nos,eps=t,Num_agents_inf=Num_agents_inf,P_ini=P_ini,N_ini=N_ini,
             mu=mu,sigma=sigma)                             
...
def Polarization():
    ...
    np.savez('FILES/Min_Polarization_'+str(sd_num)+'_'+str(bi_mean_num)+'.npz',NOS=Num_simulation,SD=SD,MU_VAL=MU_VAL,BI_MEAN=BI_MEAN,e_range=e_range,NUM_AGENTS_INF=NUM_AGENTS_INF,P_INI=P_INI,N_INI=N_INI,MU=MU,SIGMA=SIGMA)
    shutil.rmtree('DATA')
...
def Proportion():
    ...
    np.savez('FILES/Min_Diffprop_'+str(sd_num)+'_'+str(bi_mean_num)+'.npz',NOS=Num_simulation,SD=SD, BI_MEAN=BI_MEAN, Array_pos=Array_pos,Array_neg=Array_neg,e_range=e_range,NUM_AGENTS_INF=NUM_AGENTS_INF,P_INI=P_INI,N_INI=N_INI,MU=MU,SIGMA=SIGMA)   
    shutil.rmtree('DATA')
...
if __name__ == "__main__":
    ...
    for sd_num in range(len(SD)):
        ...
        M = [(i, j, k) for i, j, k in itertools.product(range(0,len(MU_VAL),1),range(0, Num_simulation, 1),range(0, neps, 1))]
        ...
        NUM_AGENTS_INFLUENCED=np.zeros((len(BI_MEAN),len(MU_VAL),Num_simulation, len(e_range)))
        EFFORT_RAND=np.zeros((len(BI_MEAN),len(MU_VAL),Num_simulation, len(e_range)))
        
        for bi_mean_num in range(len(BI_MEAN)):
            data=np.load('FILES/Min_Polarization_'+str(sd_num)+'_'+str(bi_mean_num)+'.npz')
        ...
        np.savez('VAR_FILES/Polarization_delta.npz', Num_ppl=N1,e_range=e_range,strength_w0=abs(val_w0),SD=SD[0],DELTA_vals=2*BI_MEAN, MEAN_vals=MU_VAL,EFFORT=EFFORT, NUM_AGENTS_INFLUENCED=NUM_AGENTS_INFLUENCED, EFFORT_RAND=EFFORT_RAND)    
        shutil.rmtree('FILES')
        ...
        np.savez('VAR_FILES/Mean_variation.npz', Num_ppl=N1,e_range=e_range,strength_w0=abs(val_w0),SD=SD[0],DELTA_vals=2*BI_MEAN, MEAN_vals=MU_VAL,EFFORT=EFFORT, NUM_AGENTS_INFLUENCED=NUM_AGENTS_INFLUENCED, EFFORT_RAND=EFFORT_RAND)    
        shutil.rmtree('FILES')
        ...
        MU_VAL=np.linspace(0,0.08,1)
        M = [(i, j, k) for i, j, k in itertools.product(range(0,nbias,1),range(0, Num_simulation, 1),range(0, neps, 1))]
        for bi_mean_num in range(len(BI_MEAN)):
            Array_pos=[]
            Array_neg=[]
            bi_mean=BI_MEAN[bi_mean_num]
        ...
        NUM_AGENTS_INFLUENCED=np.zeros((len(BI_MEAN),nbias, Num_simulation, len(e_range)))    
        EFFORT_RAND=np.zeros((len(BI_MEAN),nbias, Num_simulation, len(e_range)))
        
        for bi_mean_num in range(len(BI_MEAN)):
            data=np.load('FILES/Min_Diffprop_'+str(sd_num)+'_'+str(bi_mean_num)+'.npz')
            EFFORT[bi_mean_num]=data['NUM_AGENTS_INF']*abs(val_w0)
            EFFORT_RAND[bi_mean_num]=data['MU']*abs(val_w0)
            NUM_AGENTS_INFLUENCED[bi_mean_num]=data['NUM_AGENTS_INF']
        ...
        np.savez('VAR_FILES/Proportion_variation.npz', Num_ppl=N1, e_range=e_range, strength_w0=abs(val_w0),SD=SD[0],DELTA_vals=2*BI_MEAN, MEAN_vals=MU_VAL, EFFORT=EFFORT,NUM_AGENTS_INFLUENCED=NUM_AGENTS_INFLUENCED,Array_pos=Array_pos, Array_neg=Array_neg, EFFORT_RAND=EFFORT_RAND)    
        shutil.rmtree('FILES')    
