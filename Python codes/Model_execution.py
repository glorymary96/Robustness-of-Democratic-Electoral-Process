...
############################################################################
# Generates the natural opinions by calling the appropriate function. If the opinion is "Bigaussian", the mean, Delta, sd parameters have to be specified beforehand in the local environment.
...
if __name__ == "__main__":
    ...
    while epsilon > 2 or epsilon < 0:
        epsilon = float(
            input("Invalid entry! Enter confidence bound value between 0 to 2 : ")
        )
    ...
    if Num_party == 2:
        ...
        if NAT_TYPE == "Bigaussian":
            mean = float(
                input(
                    "Enter mean value of the whole distribution within the interval I= [0,0.1] : "
                )
            )
            while mean < 0 or mean > 0.1:
                mean = float(
                    input(
                        "Invalid entry! Enter mean value of the whole distribution within the interval I= [0,0.1] : "
                    )
                )
            ...
            while Delta < 0 or Delta > 1:
                Delta = float(
                    input(
                        "Invalid entry! Enter Delta value within the interval I= [0,1] : "
                    )
                )
    ...
    W = np.zeros(
        (Num_ppl, Num_party)
    )  # External influence, change the components of W to add external influence
