...
def Natural_opinion(num_type, mean1, mean2):
    if num_type == 0:
        for nos in range(NOS):
            N = np.array(
                random.sample(range(400, 500), Num_states)
            ...
            for m in range(ndelta):
                ...
                for g in range(Num_states):
                    ...
                    X.append(x0)
                    np.savez(
                        "DATA/Comp_bigauss_" + str(nos) + "_" + str(m) + ".npz", X=X
                    )
    elif num_type == 1:
        mean = 0.25
        for nos in range(NOS):
            ...
            for m in range(num_negative_bias):
                ...
                for j in range(Num_states):
                    Nn = int(
                        negative_votes[m] * N[j]
                    )  # Number of people with negative opinion
...
def calc(h):
    ...
    data = np.load(
        "DATA/Comp_bigauss_" + str(num) + "_" + str(delt) + ".npz", allow_pickle=True
    )
    X = data["X"]
    ...
    np.savez(
        "DATA/Comp_bigauss_res_" + str(h) + ".npz",
        delt=delt,
        num=num,
        eps=eps,
        P_ini=P_ini,
...
def AS_SINGLE_ELECTION(h):
    ...
    np.savez(
        "DATA/Comp_bigauss_res_single_" + str(h) + ".npz",
        delt=delt,
        num=num,
        eps=eps,
        P_ini=Pn,
...
def Polarization_with_shift():
    ...
    for shift in range(len(Shift)):
        ...
        np.savez(
            "OD1/Comp_polarization_" + str(Shift[shift]) + ".npz",
            e_range=e_range,
            delta=delta,
            P_INI=P_INI,
            N_INI=N_INI,
        ...
        np.savez(
            "OD1/Comp_polarization_single_" + str(Shift[shift]) + ".npz",
            e_range=e_range,
            delta=delta,
            P_INI=P_INI,
            N_INI=N_INI,
...
def Negative_votes_bias():
    ...
    np.savez(
        "OD1/Comp_proportion.npz",
        e_range=e_range,
        negative_votes_prop=negative_votes,
        P_INI=P_INI,
        N_INI=N_INI,
        E_MM=E_MM,
        E_MP=E_MP,
    ...
    for h in range(num_negative_bias * NOS * neps):
        ...
        P_Esingle[delt, num, eps] = data["N_effort"]
    np.savez(
        "OD1/Comp_proportion_single.npz",
        e_range=e_range,
        negative_votes_prop=negative_votes,
        P_INI=P_INI,
        N_INI=N_INI,
...
if __name__ == "__main__":
    ...
    negative_votes = np.linspace(
        negative_bias_start, negative_bias_fin, num_negative_bias
    ...
    M = [
        ...
        for i, j, k in itertools.product(
            range(0, ndelta, 1), range(0, NOS, 1), range(0, neps, 1)
        )
    ]
    ...
    M = [
        (i, j, k)
        for i, j, k in itertools.product(
            range(0, num_negative_bias, 1), range(0, NOS, 1), range(0, neps, 1)
        )
    ]
