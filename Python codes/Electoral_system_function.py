...
def States_to_be_influenced(Num_party, Num_states, NUM_VOTES, party, winner_to_be):
    args = np.setdiff1d(
        np.arange(0, Num_party, 1), winner_to_be
    )  # Parties excluding the winner_to_be
...
def Single_representative(h, MAT_INV):
    ...
    Num_states_won = np.sum(
        (np.argsort(NUM_VOTES).argsort() == Num_party - 1) * 1, axis=0
    )
    ...
    for num_party in range(1):
        ...
        States_to_influence = States_to_be_influenced(
            Num_party, Num_states, NUM_VOTES, party, winner_to_be
        )
        E = []
        INFLUENCED_STATES = []
        for state in range(len(States_to_influence)):
            ...
            Num_agents_inf, Eff, Num_votes_inf, Y = effort(
                Num_party,
                X0[state_num],
                MAT_INV[state_num],
                Result[state_num],
            ...
            Num_states_won_inf = np.sum(
                (np.argsort(NUM_VOTES_INF).argsort() == Num_party - 1) * 1, axis=0
            )
    ...
    np.savez(
        "MULTIPARTY_SR/Multipleparty_" + str(h) + ".npz",
        NUM_AGENTS_INFLUENCED=NUM_AGENTS_INFLUENCED,
        Effort=Effort,
        NUM_VOTES=NUM_VOTES,
        Tot_NUM_VOTES_INF=Tot_NUM_VOTES_INF,
        INF_STATES=INF_STATES,
        NUM_SEATS=NUM_SEATS,
        Tot_NUM_SEATS_INF=Tot_NUM_SEATS_INF,
    )
...
def Prop_seats(Num_party, Num_states, Num_seats_per_state, Num_ppl_states, NUM_VOTES):
    Temp = (
        (NUM_VOTES / Num_ppl_states.reshape(Num_states, 1))
        * Num_seats_per_state.reshape(Num_states, 1)
    ) - (
        (NUM_VOTES / Num_ppl_states.reshape(Num_states, 1))
        * Num_seats_per_state.reshape(Num_states, 1)
    ).astype(
    ...
    Num_seats = (
        NUM_VOTES
        / Num_ppl_states.reshape(Num_states, 1)
        * Num_seats_per_state.reshape(Num_states, 1)
    ).astype(int) + Additional_seats
...
def Prop_representative(h, MAT_INV):
    ...
    Num_seats_won, NUM_SEATS = Prop_seats(
        Num_party, Num_states, Num_seats_per_state, Num_ppl_states, NUM_VOTES
    )
    ...
    for num_party in range(1):
        ...
        for state in range(len(States_to_influence)):
            ...
            NUM_VOTES_INF[state_num] = Num_votes_inf
            INFLUENCED_STATES.append(state_num)
            Num_seats_won_inf, Num_seats_inf = Prop_seats(
                Num_party,
                Num_states,
                Num_seats_per_state,
                Num_ppl_states,
    ...
    np.savez(
        "MULTIPARTY_PR/Multipleparty_" + str(h) + ".npz",
        NUM_AGENTS_INFLUENCED=NUM_AGENTS_INFLUENCED,
        Effort=Effort,
        NUM_VOTES=NUM_VOTES,
        Tot_NUM_VOTES_INF=Tot_NUM_VOTES_INF,
...
def WTAR_representative(h, MAT_INV):
    ...
    Num_seats_won, NUM_SEATS = WTAR_seats(
        Num_party, Num_states, Num_seats_per_state, NUM_VOTES
    )
    # Election_winner=winner(Num_seats_won)
    # print('Winner', Num_seats_won, Election_winner)
    Effort = np.zeros((Num_party - 1))
    ...
    for num_party in range(1):
        ...
        for state in range(len(States_to_influence)):
            ...
            NUM_VOTES_INF[state_num] = Num_votes_inf
            INFLUENCED_STATES.append(state_num)
            Num_seats_won_inf, Num_seats_inf = WTAR_seats(
                Num_party, Num_states, Num_seats_per_state, NUM_VOTES_INF
            )
            Election_winner_inf = winner(Num_seats_won_inf)
            if Election_winner_inf == winner_to_be:
                break
        ...
        # print(Num_seats_won_inf,Election_winner_inf,winner_to_be)
    np.savez(
        "MULTIPARTY_WTAR/Multipleparty_" + str(h) + ".npz",
        NUM_AGENTS_INFLUENCED=NUM_AGENTS_INFLUENCED,
        Effort=Effort,
        NUM_VOTES=NUM_VOTES,
        Tot_NUM_VOTES_INF=Tot_NUM_VOTES_INF,
...
def RCV_seats(Num_party, NUM_VOTES, Num_states, Num_seats_per_state, Result):
    ...
    for num_state in range(Num_states):
        ...
        if Num_seats_per_state[num_state] > Num_party:
            NUM_SEATS[num_state] = np.floor(
                (100 * NUM_VOTES[num_state] / np.sum(NUM_VOTES[num_state])) / Threshold
            )
        else:
        ...
        Num_votes = np.sum(
            np.argsort(Result[num_state]).argsort() == Num_party - 1, axis=0
        )
        ...
        if Seats_RCV > 1:
            ...
            while S_RCV > 0:
                if np.max(Diff) > 1:
                    pos_party = np.argmax(
                        Diff
                    )  # position of the party with surplus votes
                    NUM_SEATS[num_state, pos_party] += 1
                    S_RCV -= 1

                    number_of_surplus_votes = int(
                        0.01 * int(np.max(Diff)) * len(Result[num_state])
                    )

                    # Indices of party with surplus votes
                    Max = np.argsort(Result[num_state]).argsort() == Num_party - 1
                    indices_pos_party = (
                        Max[:, pos_party] * np.arange(1, len(Result[num_state]) + 1, 1)
                        - 1
                    )
                    indices_pos_party = np.delete(
                        indices_pos_party, indices_pos_party == -1
                    )

                    Temp_surplus = Result[num_state][indices_pos_party]
                    Temp_surplus[:, pos_party] = 0
                    Proportion = np.sum(
                        (np.argsort(Temp_surplus).argsort() == Num_party - 1) * 1,
                        axis=0,
                    ) / len(Temp_surplus)

                    Surplus_prop = (
                        np.floor(Proportion) * number_of_surplus_votes
                    ).astype(int)
                    ...
                    for num_party in range(Num_party):
                        indices = (
                            Max[:, num_party] * np.arange(1, len(Temp_surplus) + 1, 1)
                            - 1
                        )
                    ...
                    for num_party in range(Num_party):
                        random_selection = np.random.choice(
                            INDICES[num_party], size=Surplus_prop[num_party]
                        )
                        y_temp[random_selection, pos_party] = 0

                    Num_votes_new = np.sum(
                        np.argsort(y_temp).argsort() == Num_party - 1, axis=0
                    )
...
def RCV_system(h, MAT_INV):
    ...
    Num_seats_won, NUM_SEATS = RCV_seats(
        Num_party, NUM_VOTES, Num_states, Num_seats_per_state, Result
    )
    # Election_winner=winner(Num_seats_won)
    # print('Winner', Num_seats_won, Election_winner)
    Effort = np.zeros((Num_party - 1))
    ...
    for num_party in range(1):
        ...
        for state in range(len(States_to_influence)):
            state_num = States_to_influence[state]
            w0 = STRG.Strategies(Num_party, winner_to_be)
            Num_agents_inf, Eff, Num_votes_inf, New_Result[state_num] = effort(
                Num_party,
                X0[state_num],
                MAT_INV[state_num],
                New_Result[state_num],
                NUM_VOTES[state_num],
                winner_to_be,
                w0,
            )
            ...
            NUM_VOTES_INF[state_num] = Num_votes_inf
            INFLUENCED_STATES.append(state_num)
            Num_seats_won_inf, Num_seats_inf = RCV_seats(
                Num_party, NUM_VOTES_INF, Num_states, Num_seats_per_state, New_Result
            )
            Election_winner_inf = winner(Num_seats_won_inf)
            if Election_winner_inf == winner_to_be:
                break
    ...
    np.savez(
        "MULTIPARTY_RCV/Multipleparty_" + str(h) + ".npz",
        NUM_AGENTS_INFLUENCED=NUM_AGENTS_INFLUENCED,
        Effort=Effort,
        NUM_VOTES=NUM_VOTES,
        Tot_NUM_VOTES_INF=Tot_NUM_VOTES_INF,
