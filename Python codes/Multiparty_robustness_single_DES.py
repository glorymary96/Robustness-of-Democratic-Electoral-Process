...
def outcome(h):
    ...
    for num_par_1 in range(Num_party):
        ...
        for num in range(
            len(INDICES[num_par_1])
        ):  # looping over the agents that belong to party num_par_1
            for num_par_2 in range(Num_party):
                Num_agents[num_par_2] = np.sum(
                    A[INDICES[num_par_1][num], :][INDICES[num_par_2]]
                )
            indices = np.setdiff1d(np.arange(0, Num_party, 1), np.argmax(Num_agents))
            if Num_agents[np.argmax(Num_agents)] > np.all(
                Num_agents[indices]
            ):  # Determining the max neighbors for the agent initially in num_par_1
                MAX_NEIGHBOURS[num_par_1, np.argmax(Num_agents)] += 1
            if (
                Num_agents[np.argmax(Num_agents)] > Num_agents[num_par_1]
            ):  # Inter neighbors
    ...
    for rank in range(Num_party):
        for num_par_1 in range(Num_party):
            PRIORITY_VOTES_PAR[rank, num_par_1] = np.sum(
                np.argsort(Y[FINAL_INDICES[num_par_1]]).argsort()
                == Num_party - rank - 1,
                axis=0,
            )
    ...
    np.savez(
        ...
        INTER_NEIGHBOURS=INTER_NEIGHBOURS,
        AGENTS_TRANSITION=AGENTS_TRANSITION,
        PRIORITY_VOTES_PAR=PRIORITY_VOTES_PAR,
        NORM_MAX_NEIGHBOURS=NORM_MAX_NEIGHBOURS,
        NORM_INTER_NEIGHBOURS=NORM_INTER_NEIGHBOURS,
        NORM_AGENTS_TRANSITION=NORM_AGENTS_TRANSITION,
    )
...
def Plurality(h, x0, Mat_inv, y, VOTING_TYPE):
    ...
    for j in range(
        ...
        Effort[j], NUM_AGENTS_INFLUENCED[j] = effort(
            Num_party, x0, Mat_inv, y, wp, Val_w0, VOTING_TYPE
        )
        party = party - 1

    np.savez(
        "MULTIPARTY/Multipleparty_plurality" + str(h) + ".npz",
        Effort=Effort,
        Num_votes=Num_votes,
...
def Ranked_choice_voting(h, x0, Mat_inv, y, VOTING_TYPE):
    ...
    np.savez(
        "MULTIPARTY/Multipleparty_RCV" + str(h) + ".npz",
        Effort=Effort,
        Num_votes=Num_votes,
        NUM_AGENTS_INFLUENCED=NUM_AGENTS_INFLUENCED,
        N_V=N_V,
...
def Two_Round_system(h, x0, Mat_inv, y, VOTING_TYPE):
    ...
    np.savez(
        "MULTIPARTY/Multipleparty_TRS" + str(h) + ".npz",
        Effort=Effort,
        Num_votes=Num_votes,
        NUM_AGENTS_INFLUENCED=NUM_AGENTS_INFLUENCED,
    )
...
if __name__ == "__main__":
    ...
    for Num_party in range(Par_start, Par_end):
        ...
        PRIORITY_VOTES_PAR = np.zeros(
            (Num_simulation, neps, Num_party, Num_party, Num_party)
        )
        EFF_PLURALITY = np.zeros((Num_simulation, neps, Num_party - 1))
        NUM_VOTES_PLURALITY = np.zeros((Num_simulation, neps, Num_party))
        NUM_AGENTS_INFLUENCED_PLURALITY = np.zeros(
            (Num_simulation, neps, Num_party - 1)
        )
        ...
        np.savez(
            ...
            EFF_PLURALITY=EFF_PLURALITY,
            e_range=e_range,
            NUM_VOTES_PLURALITY=NUM_VOTES_PLURALITY,
            NUM_AGENTS_INFLUENCED_PLURALITY=NUM_AGENTS_INFLUENCED_PLURALITY,
            EFF_RCV=EFF_RCV,
            NUM_AGENTS_INFLUENCED_RCV=NUM_AGENTS_INFLUENCED_RCV,
            NUM_VOTES_RCV=NUM_VOTES_RCV,
            NUM_NV=NUM_NV,
            EFF_TRS=EFF_TRS,
            NUM_AGENTS_INFLUENCED_TRS=NUM_AGENTS_INFLUENCED_TRS,
            NUM_VOTES_TRS=NUM_VOTES_TRS,
            MAX_NEIGHBOURS=MAX_NEIGHBOURS,
            INTER_NEIGHBOURS=INTER_NEIGHBOURS,
            AGENTS_TRANSITION=AGENTS_TRANSITION,
            PRIORITY_VOTES_PAR=PRIORITY_VOTES_PAR,
