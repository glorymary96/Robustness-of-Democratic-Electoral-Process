...
def x0_spec_gen(num_party, Num_ppl_party):
    ...
    while Total_agents < Num_ppl_party:
        ...
        for i in range(len(x)):
            x[i, np.argmax(x[i])], x[i, num_party] = (
                x[i, num_party],
                x[i, np.argmax(x[i])],
            )
...
def Nat_type(num):
    ...
    for num_party in range(Num_party):
        x0[Num_ppl_count : Num_ppl_count + Num_ppl_list[num, num_party]] = x0_spec_gen(
            num_party, Num_ppl_list[num, num_party]
        )
...
if __name__ == "__main__":
    ...
    for Num_party in range(Par_start, Par_end):
        ...
        for num in range(Num_simulation):
            ...
            while R != Num_ppl:
                Arr = np.array(
                    random.sample(range(num_range - 10, num_range + 10), Num_party)
                )
                R = np.sum(Arr)
    ...
    for Num_party in range(Par_start, Par_end):
        ...
        np.savez(
            "MULT_ORD/Multiparty_ordering_" + str(Num_party) + ".npz",
            Effort=EFF,
            e_range=e_range,
            NUM_VOTES=NUM_VOTES,
            NUM_AGENTS_INFLUENCED=NUM_AGENTS_INFLUENCED,
