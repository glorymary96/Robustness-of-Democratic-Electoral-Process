    ...
    for j in range(len(R)):
        ...
        while flag == 1 and flag_count < 100:
            ...
            while np.sum(y1 < -1) or np.sum(y1 > 1):
                y1[y1 < -1] = np.random.RandomState().normal(
                    -bi_mean, sd, np.sum(y1 < -1)
                )
                y1[y1 > 1] = np.random.RandomState().normal(
                    -bi_mean, sd, np.sum(y1 > 1)
                )
            ...
            while np.sum(y2 > 1) or np.sum(y2 < -1):
                ...
                y2[y2 < -1] = np.random.RandomState().normal(
                    bi_mean, sd, np.sum(y2 < -1)
                )
...
def calc(h):
    ...
    PN = P_ini - N_ini
    np.savez(
        "D_DATA/RDres_mid_" + str(h) + ".npz", P_ini=P_ini, N_ini=N_ini, X=X, PN=PN
    )
...
def WTAR_representative(h):
    ...
    for i in range(len(s2)):
        ...
        P1, N1 = WTA_representative(
            P_New_WTA - N_New_WTA, Num_districts_perstate, Num_districts
        )
        f = winner(P1, N1)
        if f != f1:
            break
...
def ELECTIONS(N_S, num_type, Num_states, Num_districts, Proportion, REPUB):
    ...
    np.savez(
        "US/RealDout_" + str(num_type + 1) + "_robustness_" + str(N_S) + ".npz",
        P_INI=P_INI,
        N_INI=N_INI,
        E_SR=E_SR,
        E_WTAR=E_WTAR,
...
def REPUB_CALC(Num_states, Num_districts_perstate):
    ...
    for nos in range(NOS):
        ...
        for num_state in range(Num_states):
            Repub.append(
                np.random.uniform(
                    low=45, high=55, size=Num_districts_perstate[num_state]
                )
            )
        REPUB.append(Repub)
    return REPUB
...
def SYNTHETIC_COUNTRIES():
    ...
    for N_S in range(len(NUM_STATES)):
        ...
        Num_districts_perstate = NUM_SEATS_PER_STATE[
            N_S
        ]  # Seat distribution in the states
        Num_districts = np.sum(Num_districts_perstate).astype(
            "int"
        )  # Total number of seats/districts
        N = Num_districts_perstate * NP_district
        N[
            N % 2 == 0
        ] += 1  # Number of agents in each state determined by the number of seats allotted to the corresponding state
        ...
        np.savez(
            ...
            Num_districts_perstate=Num_districts_perstate,
        ...
        for num_type in range(Num_type):
            ...
            os.mkdir("DATA")
            ELECTIONS(
                N_S, num_type, Num_states, Num_districts, Proportion, REPUB
            )  # Elections in states
            shutil.rmtree("DATA")
...
def CASE_STUDY_US():
    ...
    df_state_district_distribution = pd.read_csv(
        "HOR_DATA/state_district_distribution.csv"
    )
    Num_districts_perstate = np.array(
        df_state_district_distribution["Num_districts"], dtype=int
    )
    N = Num_districts_perstate * NP_district
    N[N % 2 == 0] += 1
    Proportion = Num_districts_perstate / N
    ...
    for year in range(2012, 2022, 2):
        ...
        Repub = (
            []
        )  # For election with different outcomes depending on state distribution
        i, j = 0, 0
        ...
        for num_type in range(Num_type):
            ...
            os.mkdir("DATA")
            ELECTIONS(
                year, num_type, Num_states, Num_districts, Proportion, REPUB
            )  # Elections in states
            shutil.rmtree("DATA")
...
def DEFINE_SYNTHETIC_COUNTRY():
    ...
    for num_state in range(Num_states):
        Num_districts_perstate[num_state] = int(
            input("Enter number of seats in district " + str(num_state + 1) + ": ")
        )
...
if __name__ == "__main__":
    ...
    neps = 51
    e_range = np.linspace(0, 1.5, neps).round(
        decimals=3
    ...
    np.savez(
        "US/Assumptions.npz",
        Number_of_simulation=NOS,
        e_range=e_range,
        Influence_strength=val_w0,
        Delta=2 * bi_mean,
        mean=mean,
        std_dev=sd,
