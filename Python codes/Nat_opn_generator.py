...
def rand_opinion_pos(p, V, k):
    perm = gen_rand_perm_vect(
        np.concatenate((np.zeros(k - 1), np.ones(p - k)))
    )  # Take one of the permutations of k-1 zeros and p-k ones.

    idx = np.zeros(p, dtype=int)
    left = (np.setdiff1d((1 - perm) * np.arange(2, p + 1, 1), np.array((0)))).astype(
        "int"
    )  # Indices of the parties at the left of party k in the order of the opinion.
    right = (np.setdiff1d((perm) * np.arange(2, p + 1, 1), np.array((0)))).astype(
        "int"
    )  # Indices of the parties at the right of party k in the order of the opinions.
