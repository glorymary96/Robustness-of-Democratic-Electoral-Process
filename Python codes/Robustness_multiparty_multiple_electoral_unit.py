...
def Nat_type(Num_party,num_state):
    ...
    Req_per=np.random.RandomState().uniform(low=100/Num_party-5,high=100/Num_party+5,size=(Num_party))
    Req_ppl=(((Req_per/np.sum(Req_per))*Num_ppl_states[num_state]).round(decimals=0)).astype('int')
    ...
    for num_party in range(Num_party):
        x0[Num_ppl_count:Num_ppl_count+Req_ppl[num_party]]=OPN_GEN.Random_generation_pos(Req_ppl[num_party], Num_party, (num_party+1))
...
def NATURALOPINION(num):
    ...
    for num_state in range(Num_states):
        ...
        X0.append(x0)
    np.savez('MULTIPARTY_NO/NO_'+str(Num_party)+'_'+str(num)+'.npz',x0=np.array(X0,dtype=object),Num_ppl_states=Num_ppl_states) 
...
def outcome(h):
    ...
    data=np.load('MULTIPARTY_NO/NO_'+str(Num_party)+'_'+str(num)+'.npz',allow_pickle=True)
    X0=data['x0']
    e=e_range[eps]
    ...
    for num_state in range(Num_states):
        x0=X0[num_state]
        NUM_VOTES_INITIAL[num_state]=np.sum((np.argsort(x0).argsort()==Num_party-1)*1,axis=0)
        A=np.zeros((len(x0), len(x0)))
        for i in range(len(x0)):
            A[i,:]=((np.linalg.norm(np.tile(x0[i],(len(x0),1))-x0,ord=1,axis=1))<e)*1
        ...
        Result.append(Y)
        NUM_VOTES[num_state]=np.sum((np.argsort(Y).argsort()==Num_party-1)*1,axis=0)
    
    np.savez('MULTIPARTY_RES/Inital_res_'+str(h)+'_.npz', Num_party=Num_party, Num_states=Num_states, Num_seats_per_state=Num_seats_per_state,
             Num_districts=Num_districts, Num_ppl_states=Num_ppl_states, X0=np.array(X0,dtype=object), Result=np.array(Result,dtype=object), NUM_VOTES=np.array(NUM_VOTES,dtype=object)
             )
...
def ELECTION_NEC():
    ...
    for h in range(Num_simulation*neps):
        data=np.load('MULTIPARTY_SR/Multipleparty_'+str(h)+'.npz',allow_pickle=True)
        num,eps=int(M[h][0]),int(M[h][1])
        EFF_SR[num,eps]=data['Effort'][0]
        NUM_AGENTS_INFLUENCED_SR[num,eps]=data['NUM_AGENTS_INFLUENCED'][0]
        
        data=np.load('MULTIPARTY_PR/Multipleparty_'+str(h)+'.npz',allow_pickle=True)
        EFF_PR[num,eps]=data['Effort'][0]
        NUM_AGENTS_INFLUENCED_PR[num,eps]=data['NUM_AGENTS_INFLUENCED'][0]
        
        data=np.load('MULTIPARTY_WTAR/Multipleparty_'+str(h)+'.npz',allow_pickle=True)
        EFF_WTAR[num,eps]=data['Effort'][0]
        NUM_AGENTS_INFLUENCED_WTAR[num,eps]=data['NUM_AGENTS_INFLUENCED'][0]
        
        data=np.load('MULTIPARTY_RCV/Multipleparty_'+str(h)+'.npz',allow_pickle=True)
        EFF_RCV[num,eps]=data['Effort'][0]
        NUM_AGENTS_INFLUENCED_RCV[num,eps]=data['NUM_AGENTS_INFLUENCED'][0]
        
    
    np.savez('Multi_States/Pol_Multiparty_diff_elec_sys_'+str(Num_party)+'_'+str(N_S)+'.npz',Num_simulation=Num_simulation,e_range=e_range,EFF_SR=EFF_SR,EFF_PR=EFF_PR,EFF_WTAR=EFF_WTAR, EFF_RCV=EFF_RCV, NUM_AGENTS_INFLUENCED_SR=NUM_AGENTS_INFLUENCED_SR, 
             NUM_AGENTS_INFLUENCED_PR=NUM_AGENTS_INFLUENCED_PR, NUM_AGENTS_INFLUENCED_WTAR=NUM_AGENTS_INFLUENCED_WTAR, NUM_AGENTS_INFLUENCED_RCV=NUM_AGENTS_INFLUENCED_RCV, )
...
if __name__ == "__main__": 
    ...
    neps=51
    e_range=np.linspace(0,2,neps).round(decimals=3) # Confidence bound parameter range
    ...
    np.savez('Multi_States/Initial_considerations.npz',NUM_STATES=NUM_STATES, NUM_SEATS_PER_STATE=NUM_SEATS_PER_STATE, 
             Num_simulation=Num_simulation,neps=neps,e_range=e_range,Num_party_start=Num_party_start,
    ...
    for N_S in range(len(NUM_STATES)):
        ...
        Num_seats_per_state=NUM_SEATS_PER_STATE[N_S] # Array of number of seats per state
        
        Num_districts=int(np.sum(NUM_SEATS_PER_STATE[N_S])) # Total number of seats which is assumed to be equal to the number of districts
        Num_ppl_states=Min_agents*Num_seats_per_state # Number of people in each state
        Num_ppl_states[Num_ppl_states % 2==0]+=1 # Increasing the number of people if it's even
